...
export async function POST(req: NextRequest) {
  try {
    const { topic, duration = "short", characterDescription = "", language = "ko", moods = [] } = await req.json();
...
- Write in English
- Engaging, compelling content${moodText ? `\n- Overall mood/tone: ${moodText} — reflect this throughout the entire script` : ""}
...
- 각 장면은 명확한 주제를 가짐
- 한국어로 작성${moodText ? `\n- 전체 무드/톤: ${moodText} — 이 분위기가 스크립트 전반에 일관되게 반영될 것` : ""}
- 흥미롭고 시청자를 사로잡는 내용
